...
// src/ACDappClient.ts
var import_connect_wallet_api = require("@cedra-labs/connect-wallet-api");
var import_web_transport = require("@cedra-labs/web-transport");
var import_ts_sdk3 = require("@cedra-labs/ts-sdk");
var import_api2 = require("@cedra-labs/api");
var import_crypto2 = require("@cedra-labs/crypto");
...
function normalizePayloadForIC(payload, cedraConfig) {
  ...
  if (cedraConfig) {
    return payload.abi !== void 0 ? (0, import_ts_sdk.generateTransactionPayloadWithABI)({ ...payload, abi: payload.abi }) : (0, import_ts_sdk.generateTransactionPayload)({ cedraConfig, ...payload });
  }
...
var import_api = require("@cedra-labs/api");
var import_crypto = require("@cedra-labs/crypto");
var import_identity_wallet_api = require("@cedra-labs/identity-wallet-api");
...
var ACPairingClient = class {
  ...
  async signTransaction(address, args, options) {
    const serializedRequestArgs = (0, import_identity_wallet_api.serializeSignTransactionRequestArgs)(args);
    ...
    return (0, import_identity_wallet_api.deserializeSignTransactionResponseArgs)(serializedResponseArgs);
  }
  // endregion
  async signAndSubmitTransaction(address, args, options) {
    const serializedRequestArgs = (0, import_identity_wallet_api.serializeSignAndSubmitTransactionRequestArgs)(args);
    try {
...
var ACDappClient = class {
  ...
  async getKeylessAccounts() {
    const serializedRequest = import_connect_wallet_api.GetConnectedAccountsRequest.serialize(
      this.dappInfo
    );
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_connect_wallet_api.GetConnectedAccountsResponse.deserialize(serializedResponse);
  ...
  async disconnect(address) {
    if (await this.isIcAccount(address)) {
      ...
      const serializedRequest = import_connect_wallet_api.DisconnectRequest.serialize(this.dappInfo);
  ...
  async connect({
    ...
    const serializedRequest = import_connect_wallet_api.ConnectRequest.serialize(
      this.dappInfo,
      requestArgs
    );
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_connect_wallet_api.ConnectResponse.deserialize(serializedResponse);
    if (response.args.status === "approved") {
      ...
      return (0, import_connect_wallet_api.makeUserApproval)({ account });
    }
    return response.args;
  }
  async signIn(args) {
    const serializedRequest = import_connect_wallet_api.SignInRequest.serialize(this.dappInfo, args);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_connect_wallet_api.SignInResponse.deserialize(serializedResponse);
    return response.args;
  }
  async signMessage(args) {
    ...
    if (await this.isIcAccount(signerAddress)) {
      const { chainId } = args;
      const network = (0, import_connect_wallet_api.chainIdToNetwork)(chainId);
      let message;
      let nonce;
      try {
      ...
      return (0, import_connect_wallet_api.makeUserApproval)({
        fullMessage,
        signature
      });
    }
    const serializedRequest = import_connect_wallet_api.SignMessageRequest.serialize(this.dappInfo, args);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_connect_wallet_api.SignMessageResponse.deserialize(serializedResponse);
    return response.args;
  }
  async signTransaction(args) {
    const normalizedArgs = "transaction" in args ? import_connect_wallet_api.SignTransactionRequest.normalizeArgs(args) : args;
    const { signerAddress } = args;
    if (await this.isIcAccount(signerAddress)) {
      const {
    ...
    const serializedRequest = import_connect_wallet_api.SignTransactionRequest.serialize(
      this.dappInfo,
      normalizedArgs
    );
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_connect_wallet_api.SignTransactionResponse.deserialize(serializedResponse);
    return response.args;
  }
  async signAndSubmitTransaction(args) {
    ...
    if (await this.isIcAccount(signerAddress)) {
      ...
      const cedraConfig = new import_ts_sdk3.CedraConfig({ network });
      let convertedArgs;
      if (feePayer !== void 0) {
    ...
    const serializedRequest = import_connect_wallet_api.SignAndSubmitTransactionRequest.serialize(
      this.dappInfo,
      args
    );
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_connect_wallet_api.SignAndSubmitTransactionResponse.deserialize(serializedResponse);
    return response.args;
  }
  // endregion
};
...
// src/ICDappClient.ts
var import_crypto3 = require("@cedra-labs/crypto");
...
var import_connect_wallet_api2 = require("@cedra-labs/connect-wallet-api");
var import_web_transport2 = require("@cedra-labs/web-transport");
var import_api3 = require("@cedra-labs/api");
var ACKeylessClient = class {
  ...
  // region Public API
  async isConnected() {
    const serializedRequest = import_connect_wallet_api2.IsConnectedRequest.serialize(this.dappInfo);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_connect_wallet_api2.IsConnectedResponse.deserialize(serializedResponse);
    return response.args;
  }
  async getConnectedAccounts() {
    const serializedRequest = import_connect_wallet_api2.GetConnectedAccountsRequest.serialize(this.dappInfo);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_connect_wallet_api2.GetConnectedAccountsResponse.deserialize(serializedResponse);
    return response.args;
  }
  async disconnect() {
    const serializedRequest = import_connect_wallet_api2.DisconnectRequest.serialize(this.dappInfo);
    await this.transport.sendRequest(serializedRequest);
  }
  async connect() {
    const serializedRequest = import_connect_wallet_api2.ConnectRequest.serialize(this.dappInfo);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_connect_wallet_api2.ConnectResponse.deserialize(serializedResponse);
    return response.args;
  }
  async signMessage(args) {
    const serializedRequest = import_connect_wallet_api2.SignMessageRequest.serialize(this.dappInfo, args);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_connect_wallet_api2.SignMessageResponse.deserialize(serializedResponse);
    return response.args;
  }
  async signTransaction(args) {
    const normalizedArgs = "transaction" in args ? import_connect_wallet_api2.SignTransactionRequest.normalizeArgs(args) : args;
    const serializedRequest = import_connect_wallet_api2.SignTransactionRequest.serialize(this.dappInfo, normalizedArgs);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_connect_wallet_api2.SignTransactionResponse.deserialize(serializedResponse);
    return response.args;
  }
  async signAndSubmitTransaction(args) {
    const serializedRequest = import_connect_wallet_api2.SignAndSubmitTransactionRequest.serialize(this.dappInfo, args);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_connect_wallet_api2.SignAndSubmitTransactionResponse.deserialize(serializedResponse);
    return response.args;
  }
  // endregion
};
