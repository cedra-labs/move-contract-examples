...
var import_connect_wallet_api6 = require("@cedra-labs/connect-wallet-api");
...
var dismissalSerializedResponse = import_connect_wallet_api.ConnectResponse.serialize({ status: "dismissed" });
  ...
  return new Promise((resolve, reject) => {
    const listeners = {
      onMessage: (message) => {
        ...
        if ((0, import_connect_wallet_api.isTypedMessage)(import_connect_wallet_api.PromptUnauthorizedErrorMessage, message.data)) {
        ...
        if ((0, import_connect_wallet_api.isTypedMessage)(import_connect_wallet_api.PromptConnectionRequestMessage, message.data)) {
          promptWindow.postMessage(new import_connect_wallet_api.PromptConnectionResponseMessage(request), baseUrl);
          return;
        }
        if ((0, import_connect_wallet_api.isTypedMessage)(import_connect_wallet_api.PromptApprovalResponseMessage, message.data)) {
          window.removeEventListener("message", listeners.onMessage);
          clearTimeout(listeners.promptPollerId);
...
function serializeLocalDappState(state) {
  ...
  for (const account of state.connectedAccounts) {
    (0, import_connect_wallet_api2.serializeAccountInfo)(serializer, account);
  }
...
function deserializeLocalDappState(serializedValue) {
  ...
  for (let i = 0; i < connectedAccountsLength; i += 1) {
    connectedAccounts.push((0, import_connect_wallet_api2.deserializeAccountInfo)(deserializer));
  }
  return { connectedAccounts };
}
function getState() {
  const encodedValue = window.localStorage.getItem(localDappStateKey);
  return encodedValue ? deserializeLocalDappState((0, import_connect_wallet_api2.base64ToBytes)(encodedValue)) : { connectedAccounts: [] };
}
function setState(state) {
  const serializedValue = serializeLocalDappState(state);
  const encodedValue = (0, import_connect_wallet_api2.bytesToBase64)(serializedValue);
...
var import_connect_wallet_api5 = require("@cedra-labs/connect-wallet-api");
var import_bridge = require("@telegram-apps/bridge");
var import_uuid = require("uuid");

// src/telegram/createWalletRequest.ts
var import_connect_wallet_api3 = require("@cedra-labs/connect-wallet-api");
async function createWalletRequest(baseUrl, request) {
  const encodedClientIdentityKey = (0, import_connect_wallet_api3.bytesToBase64url)(request.clientIdentityKey.toUint8Array());
  const response = await fetch(`${baseUrl}/v1/dapp/clients/${encodedClientIdentityKey}/requests/`, {
    body: JSON.stringify({
      body: (0, import_connect_wallet_api3.encodeWalletRequestBody)(request.body),
      id: request.id,
      signature: (0, import_connect_wallet_api3.bytesToBase64)(request.signature.toUint8Array()),
...
var import_connect_wallet_api4 = require("@cedra-labs/connect-wallet-api");
...
async function getWalletResponse(baseUrl, clientIdentityKey, requestId) {
  const encodedClientIdentityKey = (0, import_connect_wallet_api4.bytesToBase64url)(clientIdentityKey.toUint8Array());
  ...
  const data = (0, import_connect_wallet_api4.base64ToBytes)(responseBody.data.body);
  return { data };
}
...
async function openTelegramPrompt(args) {
  ...
  const challenge = (0, import_connect_wallet_api5.makePopupWalletRequestChallenge)({ body, id: requestId, timestamp });
  ...
  let encodedRequest = (0, import_connect_wallet_api5.encodePopupWalletRequest)(signedWalletRequest);
  if (encodedRequest.length > 1024) {
    void createWalletRequest(baseUrl, signedWalletRequest);
    encodedRequest = (0, import_connect_wallet_api5.encodePopupWalletRequest)({
      clientIdentityKey,
      id: requestId,
      signature,
      timestamp
...
var WebWalletTransport = class {
  ...
  async sendRequest(request) {
    switch (request.name) {
      case import_connect_wallet_api6.IsConnectedRequest.name: {
        const connectedAccounts = getConnectedAccounts();
        return import_connect_wallet_api6.IsConnectedResponse.serialize(connectedAccounts.length > 0);
      }
      case import_connect_wallet_api6.GetConnectedAccountsRequest.name: {
        const connectedAccounts = getConnectedAccounts();
        return import_connect_wallet_api6.GetConnectedAccountsResponse.serialize(connectedAccounts);
      }
      case import_connect_wallet_api6.ConnectRequest.name: {
        const serializedResponse = await this.sendPromptRequest(request);
        const response = import_connect_wallet_api6.ConnectResponse.deserialize(serializedResponse);
      ...
      case import_connect_wallet_api6.SignInRequest.name: {
        const serializedResponse = await this.sendPromptRequest(request);
        const response = import_connect_wallet_api6.SignInResponse.deserialize(serializedResponse);
        if (response.args.status === "approved") {
      ...
      case import_connect_wallet_api6.DisconnectRequest.name: {
        const [activeAccount] = getConnectedAccounts();
        if (activeAccount) {
          removeConnectedAccount(activeAccount.address);
        }
        return import_connect_wallet_api6.DisconnectResponse.serialize({});
      }
      case import_connect_wallet_api6.SignMessageRequest.name:
      case import_connect_wallet_api6.SignTransactionRequest.name:
      case import_connect_wallet_api6.SignAndSubmitTransactionRequest.name: {
