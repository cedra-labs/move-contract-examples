...
export const installCli = async () => {
    ...
    if (os === "Windows") {
        ...
        execSync(`powershell -Command "if (!(Test-Path -Path 'C:\\tmp')) { New-Item -ItemType Directory -Path 'C:\\tmp' } ; Invoke-RestMethod -Uri ${url} -OutFile C:\\tmp\\cedra.zip; Expand-Archive -Path C:\\tmp\\cedra.zip -DestinationPath C:\\tmp -Force; Move-Item -Path C:\\tmp\\cedra.exe -Destination \"${path}\""`);
    }
    else if (os === "MacOS") {
        ...
        execSync(`curl -L -o /tmp/cedra.zip ${url}; unzip -o -q /tmp/cedra.zip -d /tmp; mv /tmp/cedra ${path};`);
    }
    else {
