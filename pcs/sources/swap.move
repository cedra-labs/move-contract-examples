...
module pancake::swap {
    ...
    struct TokenPairMetadata<phantom X, phantom Y> has key {
        ...
        fee_amount: FungibleAsset,
        /// It's reserve_x * reserve_y, as of immediately after the most recent liquidity event
        k_last: u128,
        /// T0 token balance
        balance_x: FungibleAsset,
        /// T1 token balance
        balance_y: FungibleAsset,
        /// Mint capacity of LP Token
        mint_ref: MintRef,
        /// Burn capacity of LP Token
        burn_ref: BurnRef,
        /// extend_ref is used to retrieve the pair's object signer which is used when interacting with the underlying pool
    ...
    /*
     ...
     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     Please use swap_utils::sort_assets()
    ...
    */
    ...
    public(friend) fun create_pair(
        sender: &signer,
        x_metadata: Object<Metadata>,
        y_metadata: Object<Metadata>
    ) acquires SwapInfo {
        assert!(!is_pair_created(x_metadata, y_metadata), ERROR_ALREADY_INITIALIZED);
        ...
        let lp_name = generate_pair_name(x_metadata, y_metadata);
    ...
    public fun is_pair_created(x_metadata: Object<Metadata>, y_metadata: Object<Metadata>): bool {
        let pair_address = generate_pair_address(x_metadata, y_metadata);
        exists<TokenPairReserve>(pair_address)
    }

    fun generate_pair_address(
        x_metadata: Object<Metadata>,
        y_metadata: Object<Metadata>
    ): address {
        // concatenate the assets' addresses in bytes format
        let concatenated_addresses = vector::empty<u8>();
        let x_address_to_bytes = bcs::to_bytes(&object::object_address(&x_metadata));
        let y_address_to_bytes = bcs::to_bytes(&object::object_address(&y_metadata));
        vector::append(&mut concatenated_addresses, x_address_to_bytes);
        let pair_hash = aptos_hash::keccak256(concatenated_addresses);

        object::create_object_address(&RESOURCE_ACCOUNT, pair_hash)
    }

    fun generate_pair_name(
        x_metadata: Object<Metadata>,
        y_metadata: Object<Metadata>
    ): vector<u8> {
        let pair_name = string::utf8(b"PCS-");
        let x_name_bytes = swap_utils::get_asset_name_in_bytes(x_metadata);
        let y_name_bytes = swap_utils::get_asset_name_in_bytes(y_metadata);
        string::append(&mut pair_name, x_name_bytes);
        string::append_utf8(&mut pair_name, b"/");
        string::append(&mut pair_name, y_name_bytes);
    }
    ...
    public fun total_lp_supply(x_metadata: Object<Metadata>, y_metadata: Object<Metadata>): u128 {
    ...
    public fun token_reserves(x_metadata: Object<Metadata>, y_metadata: Object<Metadata>): (u64, u64, u64) acquires TokenPairReserve {
    ...
    public fun token_balances(x_metadata: Object<Metadata>, y_metadata: Object<Metadata>): (u64, u64) acquires TokenPairMetadata {
        let meta =
    ...
    fun add_liquidity_direct<X, Y>(
        ...
        let (reserve_x, reserve_y, _) = token_reserves(x_metadata, y_metadata);
        ...
        deposit_x<X, Y>(x);
        deposit_y<X, Y>(y);
        let (lp, fee_amount) = mint(x_metadata, y_metadata);
    ...
    public(friend) fun swap_exact_x_to_y_direct<X, Y>(
        ...
        let (rin, rout, _) = token_reserves(x_metadata, y_metadata);
    ...
    public(friend) fun swap_exact_y_to_x_direct<X, Y>(
        ...
        deposit_y<X, Y>(coins_in);
        let (rout, rin, _) = token_reserves(x_metadata, y_metadata);
        let amount_out = swap_utils::get_amount_out(amount_in, rin, rout);
        let (coins_x_out, coins_y_out) = swap<X, Y>(amount_out, 0);
        assert!(coin::value<Y>(&coins_y_out) == 0, ERROR_INSUFFICIENT_OUTPUT_AMOUNT);
        (coins_x_out, coins_y_out)
    ...
    fun swap<X, Y>(
        ...
        let (balance_x, balance_y) = token_balances(x_metadata, y_metadata);
    ...
    fun mint(x_metadata: Object<Metadata>, y_metadata: Object<Metadata>): (coin::Coin<LPToken<X, Y>>, u64) acquires TokenPairReserve, TokenPairMetadata {
        let metadata = borrow_global_mut<TokenPairMetadata<X, Y>>(RESOURCE_ACCOUNT);
        ...
        let total_supply = total_lp_supply(x_metadata, y_metadata);
    ...
    fun burn<X, Y>(lp_tokens: coin::Coin<LPToken<X, Y>>): (coin::Coin<X>, coin::Coin<Y>, u64) acquires TokenPairMetadata, TokenPairReserve {
        ...
        //Need to add fee amount which have not been mint.
        let total_lp_supply = total_lp_supply(x_metadata, y_metadata);
    ...
    fun mint_fee<X, Y>(reserve_x: u64, reserve_y: u64, metadata: &mut TokenPairMetadata<X, Y>): u64 {
        ...
        if (metadata.k_last != 0) {
            ...
            if (root_k > root_k_last) {
                let numerator = total_lp_supply(x_metadata, y_metadata) * (root_k - root_k_last) * 8u128;
    ...
    public entry fun withdraw_fee_noauth(x_metadata: Object<Metadata>, y_metadata: Object<Metadata>) acquires SwapInfo, TokenPairMetadata {
        let swap_info = borrow_global<SwapInfo>(RESOURCE_ACCOUNT);
