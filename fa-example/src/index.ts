...
// Debug prints
console.log("DEBUG - Module Name:", MODULE_NAME);
console.log("DEBUG - Module Path:", MODULE_FULL_PATH);
...
const checkBalance = async (aptos: Aptos, name: string, address: AccountAddress) => {
  try {
    const result = await aptos.view({
      payload: {
        function: `${MODULE_FULL_PATH}::balance`,
        typeArguments: [],
...
const example = async () => {
  ...
  try {
    ...
    const mintTxn = await aptos.transaction.build.simple({
      ...
      data: { 
        function: `${MODULE_FULL_PATH}::mint`,
    ...
    const transferTxn = await aptos.transaction.build.simple({
      ...
      data: { 
        function: `${MODULE_FULL_PATH}::transfer`,
